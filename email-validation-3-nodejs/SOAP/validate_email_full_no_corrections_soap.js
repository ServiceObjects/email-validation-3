import { EV3Response } from "../SOAP/ev3_response.js";
import { soap } from "strong-soap";

/**
 * <summary>
 * A class that provides functionality to call the ServiceObjects Email Validation 3 SOAP service's ValidateEmailFullNoCorrections endpoint,
 * performing a full suite of tests to validate and verify an email address without attempting corrections,
 * with fallback to a backup endpoint in live mode.
 * </summary>
 */
class ValidateEmailFullNoCorrectionsSoap {
    /**
     * <summary>
     * Initializes a new instance of the ValidateEmailFullNoCorrectionsSoap class with the provided input parameters,
    ...
    /**
     * <summary>
     * Asynchronously calls the ValidateEmailFullNoCorrections SOAP endpoint, attempting the primary endpoint
     ...
     */
    async validateEmailFullNoCorrections() {
        try {
     ...
     * <param name="args" type="Object">The arguments to pass to the ValidateEmailFullNoCorrections method.</param>
    ...
    _callSoap(wsdlUrl, args) {
        return new Promise((resolve, reject) => {
            soap.createClient(wsdlUrl, { timeout: this.timeoutSeconds * 1000 }, (err, client) => {
                ...
                client.ValidateEmailFullNoCorrections(args, (err, result) => {
                    if (err) return reject(err);
                    const rawData = result.ValidateEmailFullNoCorrectionsResult;
                    try {
                        if (!rawData) {
...
export { ValidateEmailFullNoCorrectionsSoap };