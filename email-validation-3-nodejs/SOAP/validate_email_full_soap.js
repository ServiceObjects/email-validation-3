...
/**
 * <summary>
 * A class that provides functionality to call the ServiceObjects Email Validation 3 SOAP service's ValidateEmailFull endpoint,
 * performing a full suite of tests including corrections, validation, and real-time SMTP checks with fallback to a backup endpoint in live mode.
 * </summary>
 */
class ValidateEmailFullSoap {
    /**
     * <summary>
     * Initializes a new instance of the ValidateEmailFullSoap class with the provided input parameters,
    ...
    /**
     * <summary>
     * Asynchronously calls the ValidateEmailFull SOAP endpoint, attempting the primary endpoint
     ...
     * <param name="args" type="Object">The arguments to pass to the ValidateEmailFull method.</param>
    ...
    _callSoap(wsdlUrl, args) {
        return new Promise((resolve, reject) => {
            soap.createClient(wsdlUrl, { timeout: this.timeoutSeconds * 1000 }, (err, client) => {
                ...
                client.ValidateEmailFull(args, (err, result) => {
                    if (err) return reject(err);
                    const rawData = result.ValidateEmailFullResult;
                    try {
                        if (!rawData) {
