...
/**
 * <summary>
 * A class that provides functionality to call the ServiceObjects Email Validation 3 SOAP service's ValidateEmailFast endpoint,
 * performing a rapid syntax, domain, and DNS-level verification of an email address with fallback to a backup endpoint in live mode.
 * </summary>
 */
class ValidateEmailFastSoap {
    /**
     * <summary>
     * Initializes a new instance of the ValidateEmailFastSoap class with the provided input parameters,
    ...
    /**
     * <summary>
     * Asynchronously calls the ValidateEmailFast SOAP endpoint, attempting the primary endpoint
     ...
     * <param name="args" type="Object">The arguments to pass to the ValidateEmailFast method.</param>
    ...
    _callSoap(wsdlUrl, args) {
        return new Promise((resolve, reject) => {
            soap.createClient(wsdlUrl, { timeout: this.timeoutSeconds * 1000 }, (err, client) => {
                ...
                client.ValidateEmailFast(args, (err, result) => {
                    if (err) return reject(err);
                    const rawData = result.ValidateEmailFastResult;
                    try {
                        if (!rawData) {
