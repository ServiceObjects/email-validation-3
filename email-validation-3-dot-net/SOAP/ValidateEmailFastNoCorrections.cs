...
{
    /// <summary>
    /// Provides functionality to call the ServiceObjects Email Validation SOAP service's ValidateEmailFastNoCorrections operation,
    /// which performs a rapid syntax, domain, and DNS-level verification of an email address without attempting to correct it.
    /// </summary>
    public class ValidateEmailFastNoCorrectionsValidation
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ValidateEmailFastNoCorrections"/> class, configuring primary and backup URLs based on the environment.
        /// </summary>
        ...
        public ValidateEmailFastNoCorrectionsValidation(bool isLive)
        {
        ...
        /// <summary>
        /// Asynchronously calls the ServiceObjects Email Validation SOAP endpoint to perform a fast validation of an email address without corrections.
        ...
        public async Task<ValidateEmailResponse> ValidateEmailFastNoCorrections(string EmailAddress, string LicenseKey)
        {
            ...
            try
            {
                ...
                ValidateEmailResponse response = await clientPrimary.ValidateEmailFastNoCorrectionsAsync(EmailAddress, LicenseKey).ConfigureAwait(false);

                if (_isLive && !IsValid(response))
                {
                    throw new InvalidOperationException("Primary endpoint returned null or a fatal TypeCode=3 error for ValidateEmailFastNoCorrections");
                }
                return response;
            }
            catch (Exception primaryEx)
            {
                try
                {
                    ...
                    return await clientBackup.ValidateEmailFastNoCorrectionsAsync(
                        EmailAddress, 
                        LicenseKey
                ...
                {
                    throw new Exception(
                        $"Both primary and backup endpoints failed for ValidateEmailFastNoCorrections.\n" +
