...
{
    /// <summary>
    /// Provides functionality to call the ServiceObjects Email Validation SOAP service's ValidateEmailFullNoCorrections operation,
    /// which validates and verifies an email address via a full suite of tests without attempting any corrections.
    /// </summary>
    public class ValidateEmailFullNoCorrectionsValidation
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ValidateEmailFullNoCorrections"/> class, configuring primary and backup URLs based on the environment.
        /// </summary>
        ...
        public ValidateEmailFullNoCorrectionsValidation(bool isLive)
        {
        ...
        /// <summary>
        /// Asynchronously calls the ServiceObjects Email Validation SOAP endpoint to perform a comprehensive validation of an email address without corrections.
        ...
        public async Task<ValidateEmailResponse> ValidateEmailFullNoCorrections(string EmailAddress, string LicenseKey)
        {
            ...
            try
            {
                ...
                ValidateEmailResponse response = await clientPrimary.ValidateEmailFullNoCorrectionsAsync(EmailAddress, LicenseKey).ConfigureAwait(false);

                if (_isLive && !IsValid(response))
                {
                    throw new InvalidOperationException("Primary endpoint returned null or a fatal TypeCode=1 error for ValidateEmailFullNoCorrections");
                }
                return response;
            }
            catch (Exception primaryEx)
            {
                try
                {
                    ...
                    return await clientBackup.ValidateEmailFullNoCorrectionsAsync(
                        EmailAddress, 
                        LicenseKey
                ...
                {
                    throw new Exception(
                        $"Both primary and backup endpoints failed for ValidateEmailFullNoCorrections.\n" +
