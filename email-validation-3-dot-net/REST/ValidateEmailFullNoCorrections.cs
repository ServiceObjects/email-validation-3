...
{
    /// <summary>
    /// Provides functionality to call the ServiceObjects Email Validation 3 REST API's ValidateEmailFullNoCorrections endpoint,
    /// which validates and verifies an email address via a full suite of tests without attempting any corrections.
    /// </summary>
    public class ValidateEmailFullNoCorrectionsClient
    {
        ...
        /// <summary>
        /// Synchronously calls the ValidateEmailFullNoCorrections REST endpoint to validate an email address,
        ...
        public static EV3Response Invoke(ValidateEmailFullNoCorrectionsInput input)
        {
        ...
        /// <summary>
        /// Asynchronously calls the ValidateEmailFullNoCorrections REST endpoint to validate an email address,
        /// attempting the primary endpoint first and falling back to the backup if the response is invalid
        /// (Error.TypeCode == "3") in live mode.
        /// </summary>
        /// <param name="input">The input parameters including email address, license key, and service options.</param>
        /// <returns>Deserialized <see cref="EV3Response"/> containing the validation results.</returns>
        public static async Task<EV3Response> InvokeAsync(ValidateEmailFullNoCorrectionsInput input)
        {
            //Use query string parameters so missing/options fields don't break
            //the URL as path parameters would.
            string url = BuildUrl(input, input.IsLive ? LiveBaseUrl : TrialBaseUrl);
        ...
        public static string BuildUrl(ValidateEmailFullNoCorrectionsInput input, string baseUrl)
        {
            var qs = $"JSON/ValidateEmailInfo/Full/NoCorrections/{HttpUtility.UrlEncode(input.EmailAddress)}/" +
                     $"{HttpUtility.UrlEncode(input.LicenseKey)}?format=json";
            return baseUrl + qs;
        }

        private static bool IsValid(EV3Response response) => response?.Error == null || response.Error.TypeCode != "3";

        /// <summary>
        /// Represents the input parameters for the ValidateEmailFullNoCorrections REST API call.
        /// </summary>
        ...
        public record ValidateEmailFullNoCorrectionsInput(
            string EmailAddress = "",
            string LicenseKey = "",
            bool IsLive = true,
