...
{
    public class ValidateEmailFullRestSdkExample
    {
        public static void Go(string licenseKey, bool isLive)
        {
            Console.WriteLine("\r\n-------------------------------------------------------");
            Console.WriteLine("Email Validation 3 - ValidateEmailFullInput - REST SDK");
            Console.WriteLine("-------------------------------------------------------");

            ValidateEmailFullClient.ValidateEmailFullInput validateEmailFullInput = new
            (
                EmailAddress: "Robet@gmail.com",
            ...
            Console.WriteLine("\r\n* Input *\r\n");
            Console.WriteLine($"Email Address  : {validateEmailFullInput.EmailAddress}");
            Console.WriteLine($"License Key    : {validateEmailFullInput.LicenseKey}");
            Console.WriteLine($"Is Live        : {validateEmailFullInput.IsLive}");
            Console.WriteLine($"Timeout Seconds: {validateEmailFullInput.TimeoutSeconds}");

            EV3Response response = ValidateEmailFullClient.Invoke(validateEmailFullInput);
