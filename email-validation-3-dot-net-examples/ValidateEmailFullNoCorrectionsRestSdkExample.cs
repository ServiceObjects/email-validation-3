...
{
    public class ValidateEmailFullNoCorrectionsRestSdkExample
    {
        public static void Go(string licenseKey, bool isLive)
        {
            Console.WriteLine("\r\n-------------------------------------------------------------------");
            Console.WriteLine("Email Validation 3 - ValidateEmailFullNoCorrectionsInput - REST SDK");
            Console.WriteLine("-------------------------------------------------------------------");

            ValidateEmailFullNoCorrectionsClient.ValidateEmailFullNoCorrectionsInput validateEmailFullNoCorrectionsInput = new
            (
                EmailAddress: "JayShah@gmail.com",
            ...
            Console.WriteLine("\r\n* Input *\r\n");
            Console.WriteLine($"Email Address  : {validateEmailFullNoCorrectionsInput.EmailAddress}");
            Console.WriteLine($"License Key    : {validateEmailFullNoCorrectionsInput.LicenseKey}");
            Console.WriteLine($"Is Live        : {validateEmailFullNoCorrectionsInput.IsLive}");
            Console.WriteLine($"Timeout Seconds: {validateEmailFullNoCorrectionsInput.TimeoutSeconds}");

            EV3Response response = ValidateEmailFullNoCorrectionsClient.Invoke(validateEmailFullNoCorrectionsInput);
